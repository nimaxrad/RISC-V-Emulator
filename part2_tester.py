...
def run_test(name):
    ...
    for k in range(0, max_num_instructions):
        ...
        for i in range(0, 9):
            ...
            print(ref_line)
            print(student_line)
            # If ref_trace has instructions which are in this format 00001000: addi	x28, x0, 8
            # skip this line. Its not a register dump
            if ":" in ref_line:
                continue
            # handle invalid instructions
            if "Invalid" in student_line:
                print(
                    "ERROR: found an invalid instruction in the student trace file. Line: " + student_line)
                return False

            # handle end of files
            student_finished = "exiting" in student_line or len(
                student_line) == 0
            ...
            for j in range(0, 4):
                ...
                if student_val - student_registers[(i-1)*4+j] != ref_val - ref_registers[(i-1)*4+j] and ref_val != student_val:
                    pass_bool = False
                    print("ERROR: instruction {}, register {}. Expected: 0x{:08x}, Actual: 0x{:08x}".format(
                        k, (i-1)*4+j, ref_val, student_val))

                ref_registers[(i-1)*4+j] = ref_val
                student_registers[(i-1)*4+j] = student_val
